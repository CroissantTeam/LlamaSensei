...
def test_get_transcripts_no_files(deepgram_client):
    with patch('builtins.print') as mock_print:
        deepgram_client.get_transcripts([])
    mock_print.assert_called_once_with("There is no file to transcribe")

def test_get_transcripts_existing_file(deepgram_client, tmp_path):
    existing_file = tmp_path / "existing.json"
    existing_file.touch()
    
    with patch('os.path.exists', return_value=True), \
         patch('builtins.print') as mock_print:
        deepgram_client.get_transcripts([str(existing_file)])
...
def test_transcribe_success(deepgram_client):
    mock_deepgram_client = MagicMock()
    mock_deepgram_client.listen.rest.v.return_value.transcribe_file = Mock()
    ...
    with patch('llama_sensei.backend.add_courses.speech_to_text.transcript.DeepgramClient', return_value=mock_deepgram_client), \
        ...
        deepgram_client.transcribe("test.wav", "test.json")
    ...
    mock_deepgram_client.listen.rest.v.return_value.transcribe_file.assert_called_once()


def test_transcribe_exception(deepgram_client):
