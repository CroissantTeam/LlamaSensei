...
class TextPreprocessor:
    ...
    def preprocess_text(
        self, chunks: List[tuple], apply_lemmatize=True, apply_stem=True
    ) -> List[tuple]:
        return [
            (
                self._preprocess(
                    chunk[0], apply_lemmatize=apply_lemmatize, apply_stem=apply_stem
                ),
                chunk[1],
                chunk[2],
            )
    ...
    def _preprocess(self, text, apply_lemmatize=True, apply_stem=True):
        ...
        words = [
            [word for word in sentence if word.lower() not in self.stop_words]
        ...
        if apply_lemmatize:
            pos_tagged = [
                [self.lemmatizer.lemmatize(word, pos='v') for word, tag in sentence]
        ...
        if apply_stem:
            pos_tagged = [
                [self.stemmer.stem(word) for word in sentence]
                for sentence in pos_tagged
            ]
