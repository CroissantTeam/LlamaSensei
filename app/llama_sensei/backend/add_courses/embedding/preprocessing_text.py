...
def preprocess_transcript(text, apply_lemantize=True, apply_stem=True):
    ...
    if apply_lemantize:
        pos_tagged = [
            [lemmatizer.lemmatize(word, pos='v') for word, tag in sentence]
            for sentence in pos_tagged
        ]
    else:
        pos_tagged = [[word for word, _ in sentence] for sentence in pos_tagged]
    if apply_stem:
        pos_tagged = [
            [stemmer.stem(word) for word in sentence] for sentence in pos_tagged
        ]
