...
def create_course_ui():
    ...
    st.sidebar.button(
        label="Back",
        on_click=lambda: st.session_state.update({'current_view': 'home'}),
        key="create back",
    )
    ...
    with col2:
        with st.form(key='upload_form'):
            ...
            if upload_button:
                if url:
                    upload_course_data(
                        st.session_state.course_name, url, st.session_state.erase_db
                    )
                else:
...
def choose_course_ui():
    ...
    with col1:
        with st.container(border=True):
            ...
            if st.button(
                "Erase all data currently in course database", key="remove_all"
            ):
        ...
        video_set = list(
            {
                ...
                for x in col.get()['metadatas']
            }
        )
        table = {'Title': [yad.extract_title(x) for x in video_set], 'URL': video_set}
    ...
    # Choose Course View
        ...
        with col1:
            if st.button(
                ...
                on_click=lambda: st.session_state.update({'current_view': 'choose'}),
                key="col1",
            ):
        ...
        with col2:
            if st.button(
                "Create New Course",
                on_click=lambda: st.session_state.update({'current_view': 'create'}),
                key="col2",
            ):
                st.rerun()  # Ensure the UI updates immediately
